...
class FlowEulerTrainingArgs(BaseTrainingArgs):
    ...
    def __post_init__(self):
        super().__post_init__()
        if getattr(self, "snapshot_only", False) and not (self.total_steps or self.total_grad_steps):
...
class FlowEuler:
    ...
    def denoise_autoreg(self, model, ep_length, seq_length, diffusion_steps, stable_t, cond, x_start, repeat_cond=False,
        ...
        if not isinstance(x_start, torch.Tensor):
            raise TypeError("x_start must be a tensor")
        ...
        for i in tqdm(range(Tprompt, ep_length), desc="Autoreg diffusion", disable=not verbose):
            ...
            noise = torch.randn(B, 1, *x_shape, device=device)
            noise = torch.clamp(noise, -self.cfg.noise_abs_max, self.cfg.noise_abs_max)
            ...
            for noise_idx in range(0, diffusion_steps):
                ...
                if from_noise and i == 0:
                    ...
                    added_noise = torch.randn_like(x_curr[:, :-1])
                    added_noise = torch.clamp(
                        added_noise, -self.cfg.noise_abs_max, self.cfg.noise_abs_max
                    )
                    x_curr[:, :-1] = self.diffuse(x_curr[:, :-1], t[:, :-1], added_noise)

                if denoiser_pred == 'v':
                    pred_v = model(x_curr, t, cond_i)
                elif denoiser_pred == 'x0':
                    pred_x_0 = model(x_curr, t, cond_i)
                    pred_v = self.get_v_from_xt(x_curr, pred_x_0, t)
                else:
                    raise ValueError(f"Unknown denoiser_pred: {denoiser_pred}")
                x[:, -1:] = self.sample_xprev(x_curr, pred_v, t, t_prev)[:, -1:]

                if store_intermediate_steps:
                    if denoiser_pred == 'v':
                        pred_x_0 = self.sample_x0(
                            x_curr[:, -1:], t[:, -1:], pred_v[:, -1:]
                        )
                    else:
                        pred_x_0 = pred_x_0[:, -1:]
                    ret["pred_x_0"].append(pred_x_0.cpu())
