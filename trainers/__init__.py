...
class BaseTrainingArgs:
    ...
    wandb_project_name: str = "discretize-trick"
    ...
    resume: str = "auto"
    """Use the latest matching run, a checkpoint directory, or `disable`."""
...
class BaseTrainer:
    ...
    def train(self):
        ...
        while self.step < self.cfg.total_steps:
            ...
            if self.i_snapshot > 0 and self.step % self.i_snapshot == 0 and self.accelerator.sync_gradients:
                self.accelerator.wait_for_everyone()
                if self.accelerator.is_main_process:
                    logger.info(f"Snapshotting model at step {self.step} ...")
                    self.snapshot()
                self.accelerator.wait_for_everyone()
    ...
    def init_logging(self):
        ...
        self.wandb_run = None
        if self.accelerator.is_main_process and self.cfg.track_with_wandb:
            import wandb
            ...
            self.wandb_run = wandb.init(
                ...
                id=self.cfg.run_id,
                resume="allow" if self.cfg.loaded_ckpt is not None else None,
            )
            self.wandb_run.define_metric("trainer/grad_step")
            self.wandb_run.define_metric("*", step_metric="trainer/grad_step")

    def log_wandb(self, values, step=None):
        if self.wandb_run is not None:
            values = {**values, "trainer/grad_step": self.grad_step if step is None else step}
    ...
    def init_directories(self):
        ...
        self.ckpt_dir = self.local_output_dir / "checkpoints"
    ...
    @property
    def i_snapshot(self):
        return getattr(self.cfg, "i_snapshot", 0) * self.cfg.grad_accumulation_steps
